...
namespace lynx
{
vertex_buffer::vertex_buffer(const ref<const device> &dev, const std::vector<vertex2D> &vertices) : m_device(dev)
{
    create_vertex_buffers(vertices);
}

vertex_buffer::vertex_buffer(const ref<const device> &dev, const std::vector<vertex3D> &vertices) : m_device(dev)
{
    create_vertex_buffers(vertices);
}
