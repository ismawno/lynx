...
thin_line<Dim>::thin_line(const vec_t &p1, const vec_t &p2, const lynx::color &color)
    : modelable<Dim>(context_t::device(), model_t::line(color, color)), m_p1(p1), m_p2(p2), m_transform(as_transform())
{
}
template <Dimension Dim>
thin_line<Dim>::thin_line(const vec_t &p1, const vec_t &p2, const lynx::color &color1, const lynx::color &color2)
    : modelable<Dim>(context_t::device(), model_t::line(color1, color2)), m_p1(p1), m_p2(p2),
...
{
    drawable_t::default_draw(win, this->m_model, m_transform.center_scale_rotate_translate4(), topology::LINE_LIST);
}
...
{
    return this->m_model->vertex(1).color;
}
...
{
    this->m_model->vertex_data()[0].color = color1;
}
template <Dimension Dim> void thin_line<Dim>::color2(const lynx::color &color2)
{
    this->m_model->vertex_data()[1].color = color2;
}
...
line_strip<Dim>::line_strip(const std::vector<vec_t> &points, const lynx::color &color)
    : modelable<Dim>(context_t::device(), to_vertex_array<vec_t, vertex_t>(points, color))
{
}
template <Dimension Dim>
line_strip<Dim>::line_strip(const std::vector<vertex_t> &points) : modelable<Dim>(context_t::device(), points)
{
}
...
{
    drawable_t::default_draw(win, this->m_model, m_transform.center_scale_rotate_translate4(), topology::LINE_STRIP);
}
...
{
    KIT_ASSERT_ERROR(index < this->m_model->vertex_count(),
                     "Index exceeds model's vertices count! Index: {0}, vertices: {1}", index,
                     this->m_model->vertex_count())
    return this->m_model->vertex(index);
}
...
template <Dimension Dim> vertex<Dim> &line_strip<Dim>::operator[](const std::size_t index)
{
    KIT_ASSERT_ERROR(index < this->m_model->vertex_count(),
                     "Index exceeds model's vertices count! Index: {0}, vertices: {1}", index,
                     this->m_model->vertex_count())
    return this->m_model->vertex_data()[index];
}
